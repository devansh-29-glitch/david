// Commands must be at module level, NOT inside a nested mod block.
// Tauri's generate_handler! macro cannot find commands inside nested mods.
    ...
    match client.register(&name, &email, &password).await {
        Ok(resp) => {
            ...
            *state.user_name.lock().unwrap() = name;
            ...
            Ok(serde_json::json!({"token": resp.token, "user": resp.user}))
        }
...
pub async fn send_chat(
    ...
    let client = BackendClient::new(BACKEND_URL);
    match client.send_chat(&token, &message, screenshot.as_deref(), &session_id, audio).await {
        Ok(resp) => Ok(serde_json::json!({"message": resp.message, "audio_b64": resp.audio_b64, "mode": resp.mode})),
        Err(e) => Err(e.to_string()),
    }
}
...
pub async fn reset_session(
    ...
    let session_id = state.session_id.clone();
    reqwest::Client::new()
        .post(format!("{}/david/reset?session_id={}", crate::BACKEND_URL, session_id))
